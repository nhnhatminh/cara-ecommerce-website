...
document.addEventListener("DOMContentLoaded", function() {
    // Chỉ tải danh sách sản phẩm nếu đang ở Tab Products
    // Tuy nhiên vì Single Page, ta có thể gọi loadProducts khi bấm chuyển Tab
    // Ở đây ta gọi mặc định nếu bảng tồn tại
...
function loadProducts() {
    ...
    .then(data => {
        if(data.status === 'success') {
            ...
            // Cập nhật số lượng trên Dashboard
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <button class="btn-action btn-edit" onclick="openProductModal(${p.id})"><i class="fas fa-edit"></i> Sửa</button>
                                <button class="btn-action btn-delete" onclick="deleteProduct(${p.id})"><i class="fas fa-trash"></i> Xóa</button>
                            </div>
                        </td>
                    </tr>
                `;
...
function openProductModal(id = null) {
    ...
    if (id) {
        // Chế độ SỬA: Lấy dữ liệu cũ đổ vào
        ...
        document.getElementById('p_id').value = ""; // Xóa ID
    }
...
function saveProduct(e) {
    ...
    const form = document.getElementById('product-form');
    const formData = new FormData(form); // Tự động lấy cả file ảnh
